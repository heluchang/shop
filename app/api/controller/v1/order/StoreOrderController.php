...
class StoreOrderController
{
    ...
    public function create(Request $request, StoreBargainServices $bargainServices, StorePinkServices $pinkServices, StoreOrderCreateServices $createServices, StoreSeckillServices $seckillServices, UserInvoiceServices $userInvoiceServices, StoreOrderInvoiceServices $storeOrderInvoiceServices, StoreCombinationServices $combinationServices, $key)
    {
        ...
        if ($orderId) {
            file_put_contents('test1.log', $payType);
            switch ($payType) {
                ...
                case PayServices::ALIAPY_PAY:
                    ...
                    if (bcsub((string)$orderInfo['pay_price'], '0', 2) <= 0) {
                        ...
                        $info['pay_key'] = $payKey;
                        file_put_contents('test.log', $payKey);
    ...
    {
        [$uni, $paytype, $from, $quitUrl,$openid, $type] = $request->postMore([
            ['uni', ''],
            ['paytype', 'weixin'],
            ['from', 'weixin'],
            ['quitUrl', ''],
            ['openid', ''],// 新加的参数，如果没有登录的，通过获取过来的
            ['type', 0]
        ], true);
        if (!$uni) return app('json')->fail('参数错误!');
        ...
        $orderInfo->save();
        $order = $this->services->get(['order_id' => $uni]);trace($order);
        if (!$order)
        ...
        // file_put_contents('test1.log', $order['pay_type']);
        switch ($order['pay_type']) {
            case PayServices::WEIXIN_PAY:
                $order['openid'] = $openid;
                $jsConfig = $payServices->orderPay($order->toArray(), $from);
                file_put_contents('pay.log', json_encode($jsConfig));
                if ($from == 'weixinh5') {
            ...
            case PayServices::ALIAPY_PAY:
                ...
                file_put_contents('test'.$payKey.'.log', json_encode(['order_id' => $order['order_id'], 'other_pay_type' => false]));
    ...
    {
        // $orderCache = file_put_contents('test1.log', $key);
        // $orderCache = file_get_contents('test'.$key.'.log');
        // $orderCache = json_decode($orderCache, true);
        // $orderCache['order_id'] = $orderCache;
        // file_put_contents('test33.log', $orderCache);
        ...
        // file_put_contents('test3.log', $payType);
        $orderInfo = null;
        
        if ($payType) {
            $orderInfo = $services->getOne(['order_id' => $orderCache['order_id'], 'is_del' => 0, 'paid' => 0]);
        } else {
            // print_r($orderCache['order_id']);
            $order_id = explode('_',$orderCache['order_id']);
            // print_r($order_id[1]);
            // die;
            ...
            // $orderInfo = $this->services->get(['order_id' => $orderCache['order_id']]);
        }
        // if (!$orderInfo) {
        //     $orderCache = file_get_contents('test'.$key.'.log', $key);
        //     if ($payType) {
        //         $orderInfo = $services->getOne(['order_id' => $orderCache['order_id'], 'is_del' => 0, 'paid' => 0]);
        //     } else {
        //         $orderInfo = $this->services->get(['order_id' => $orderCache['order_id'], 'paid' => 0, 'is_del' => 0]);
        ...
        // file_put_contents('test.log', $orderInfo);
    ...
    public function friendDetail(Request $request)
    {
        ...
        $analysis_settings[] = ['name'=>'代付1','title'=>sys_config('set1_title'),'content'=>sys_config('set1_content')];
        $analysis_settings[] = ['name'=>'代付2','title'=>sys_config('set2_title'),'content'=>sys_config('set2_content')];
        $analysis_settings[] = ['name'=>'代付3','title'=>sys_config('set3_title'),'content'=>sys_config('set3_content')];
        return app('json')->successful(['info'=>$info,'analysis_settings'=>$analysis_settings]);
