...
class OrderPayServices
{
    ...
    public function orderPay(array $orderInfo, string $payType)
    {
        ...
        if (!in_array($payType, ['weixinh5', 'pc']) && !request()->isApp()) {
            ...
            // $openid = 'o_5DJ58aaEEQwSeJ0040C8htbvgc';
            if (!$openid) {
                if(!isset($orderInfo['openid']) && empty($orderInfo['openid'])){
        ...
        if(sys_config('diy_pay_sp_station_open') && sys_config('diy_pay_sp_title')){
            $body = sys_config('diy_pay_sp_title');
        }
        if (!$body) {
